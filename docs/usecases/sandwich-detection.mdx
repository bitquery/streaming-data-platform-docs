---
sidebar_position: 3
title: "Detect Sandwich Attack Opportunities"
description: "Build Detect Sandwich Attack Opportunities: a practical Bitquery tutorial with GraphQL examples, streams, and runnable application code."
---
import VideoPlayer from "../../src/components/videoplayer.js";

# How to Detect Sandwich Opportunities - Tutorial
...
## Tutorial 
This is a tutorial to build a simple webpage using PHP code that connects to the Bitquery API and retrieves data for sandwich attack opportunities on the Ethereum network. 
The code then displays the data on a webpage built using PHP and HTML.

## GraphQL query

For this tutorial, we will use [this](https://ide.bitquery.io/Sandwitch-Opportunity_1) query given below. The sandwich attack observes the DEX Trade transaction in Mempool with a large purchase
...
Next, the code connects to the Bitquery API using the HTTP_Request2 library and retrieves data on the latest sandwich opportunities across all EVM chains (in support) using a GraphQL query. 
...
$sandwichOpportunityData = $responseData['data']['EVM']['DEXTrades']
```

The code retrieves an array of necessary data to execute trade from the response data and stores it in the sandwichOpportunityData variable.
...
```php
echo '<html><body>';
echo '<h1>Sandwich MEV Opportunities</h1>';
...
<video controls loop muted playsInline width="100%" src="/img/sandwich.mp4"></video>
...
## Video Tutorial on How to Detect Sandwich Opportunities
