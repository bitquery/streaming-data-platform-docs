...
Monitoring blockchain activities is crucial for developers, investors, and regulatory bodies. Tools like [Bitquery](https://bitquery.io) offer the infrastructure to monitor blockchain activities effectively. In this tutorial, we'll use [Bitquery Solana real-time data](https://bitquery.io/blockchains/solana-blockchain-api) and Python to build a real-time Solana DEX monitoring dashboard.
...
Use Streamlit's sidebar to allow users to choose between different datasets ('' or 'Raydium'):

```python
page = st.sidebar.radio("Select Page", ["", "Raydium"])
_df = pd.DataFrame()
...
if page == "":
    ...
    while True:
        async for result in transport.subscribe(
            ...
            if result.data:
                new_data = pd.json_normalize(result.data['Solana'][''])
...
```python
def main():
    st.title("Solana DEX  & Raydium Data Dashboard")
