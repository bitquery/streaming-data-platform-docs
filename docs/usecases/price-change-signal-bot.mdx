...
description: "Bitquery how-to guide: How to build a Crypto Price Change Signal Bot on Telegram."
---

# How to build a Crypto Price Change Signal Bot on Telegram

This bot fetches real-time trading data from Bitquery and sends alerts on significant token price changes. For pre-aggregated price data with OHLC, consider using our [Crypto Price API](https://docs.bitquery.io/docs/trading/crypto-price-api/introduction/). The bot is built using the `python-telegram-bot` library to handle interactions with Telegram users and `aiohttp` for making asynchronous API requests.
...
2. **Configuration and Constants**
   ...
   - `OAUTH_TOKEN`: The authorization token for the Bitquery API. Check out the steps on how to get it [here](https://docs.bitquery.io/docs/authorization/how-to-generate/)
