...
You can also get trades for specific token using DEXTradeByTokens API. Here is [an example](https://ide.bitquery.io/Sunpump-trades-by-token).
...
```
{
  Tron {
    DEXTradeByTokens(
      where: {Trade: {Side: {Type: {is: buy}}, Currency: {SmartContract: {is: "TN7DUvYaAf9RfZiYjup7KvvC9bJDRH48vj"}}}}
    ) {
      Trade {
        Amount
        AmountInUSD
