...
## Price Change for an Uniswap Pair

[This](https://ide.bitquery.io/Price-change-percent-for-uniswap-pair-5-minutes) API endpoint returns the price change percentage for a token pair on `Uniswap` over a period of `5 minutes`.
...
query MyQuery {
  Trading(dataset: realtime) {
    Pairs(
      where: {Market: {Network: {is: "Ethereum"}, ProtocolFamily: {is: "Uniswap"}, Program: {is:"0xc90b20b897a17c28a98c03a8bbc62e03ceccb62b"}}, Interval: {Time: {Duration: {eq: 300}}}}
      orderBy: {descending: Interval_Time_End}
      limit: {count: 1}
    ) {
      Interval {
      ...
      calculate(expression: "100 * ( $Price_Ohlc_Close - $Price_Ohlc_Open ) / $Price_Ohlc_Open")
    }
  }
}
```
