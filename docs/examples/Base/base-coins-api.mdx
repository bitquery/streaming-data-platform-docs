...
[This](https://ide.bitquery.io/Latest-Coin-on-Base-Coin_2) query returns the details of all the tokens and Memecoins created on the Base Network for a particular day.
  ...
  EVM(network: base) {
    Calls(
      where: {
        ...
        Block: {
          Time: {
            after: "2024-07-10T00:00:00Z"
            before: "2024-07-10T23:59:59Z"
          }
        }
      }
    ) {
...
[This](https://ide.bitquery.io/OHLC-of-AERO-Coin) query returns the OHLC parameters and `volume` for the AERO/USDC trade pair for a one hour window.
