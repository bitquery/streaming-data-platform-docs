...
You can also access a live stream of Polymarket trades in real time. See this [streaming example](https://ide.bitquery.io/polymarket-trades-stream).
...
subscription{
  EVM(network: matic) {
    DEXTradeByTokens(
      where: {TransactionStatus: {Success: true}, 
        Trade: {Side: {Currency: {SmartContract: 
...
Use the condition ID to query `ConditionPreparation` events from the Main Polymarket Contract to get the question ID.

### Step 3: Get Question Metadata from UMA Adapter

Use the question ID to query `QuestionInitialized` events from the UMA Adapter contract to get the question metadata (ancillaryData).
