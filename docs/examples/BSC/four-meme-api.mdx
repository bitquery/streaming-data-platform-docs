...
Every bit of onchain information related to the Four Meme Exchange on BSC, live trades, bonding curve progress, newly created tokens, prices, volumes, makers/buyers/sellers, 
and more can also be accessed directly through Bitquery APIs and streams. With our GraphQL APIs, real-time WebSocket feeds, 
and enterprise-grade Kafka topics (plus optional cloud connectors like AWS, GCP, and Snowflake), you get seamless access to the 
same on-chain data powering the exchange, ready for your analytics and trading workflows. Create your account and get started by following the 
[Quickstart instructions](https://docs.bitquery.io/docs/start/first-query/).
...
- [BSC Pancake Swap APIs](https://docs.bitquery.io/docs/examples/BSC/pancake-swap-api/): Pancake swap Trades on BSC network with the help of examples.
...
query MyQuery($currency: String) {
  EVM(network: bsc) {
    DEXTradeByTokens(
      ...
      trades_24hr: count
      trades_1hr: count(if: {Block: {Time: {since_relative: {hours_ago:1}}}})
