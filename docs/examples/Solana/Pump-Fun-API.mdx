...
[This](https://ide.bitquery.io/mcap-liquidity-volume-of-a-token-in-time-frame) query returns important trade metrics like marketcap,
liquidity and trade volume of a Pump Fun token for a time frame of one hour. This implies the results returns marketcap and liquidity
`1 hour` ago along with the traded volume in the last one hour. The volume and liquidity are returned against a token which is `USDC`
in this case.
