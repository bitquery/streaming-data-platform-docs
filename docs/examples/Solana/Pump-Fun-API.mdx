# Pump Fun API - Live Prices, OHLCV, ATH, MarketCap, Liquidity

Get Ultra low latency Prices, OHLCV, ATH, MarketCap, Liquidity, traders and other information from PumpFun API and Streams.

Bitquery provides Pumpfun data through APIs, Streams and Data Dumps. 

The below graphQL APIs and Streams are examples of data points you can get with Bitquery. If you have any question on other data points reach out to support
...
[Run Query ➤](https://ide.bitquery.io/Pumpfun-token-prices-against-sol_2)
...
subscription {
  Trading {
    Pairs(
      where: {Price: {IsQuotedInUsd: false}, Market: {Network: {is: "Solana"},
...
Graphql Stream for PumpFun Token prices against USD 

[Run Query ➤](https://ide.bitquery.io/Pumpfun-token-prices-against-USD)
...
Fetches the most recent price data for a specified token usdin DexTradeByTokens
...
Live stream of token price updates on Pump Fun using Price Index
[Run Stream ➤](https://ide.bitquery.io/Price-of-a-pump-fun-token-using-price-index-in-usd)
...
subscription {
  Trading {
    Pairs(
      where: {
        Token:{Address:{is:"BUWg5Fhzvwn2xm4EYoaSFGNZXFRk8ThYe9h5R5GaXipE"}}
...
Live stream of token price updates on Pump Fun using DexTradeByTokens
...
Following is the example of query using Price Index to get top coins based on price in last 5 minutes.
...
Use the below query to get Top 10 pump fun tokens by Price change in last 5mins using DexTradingByTokens. Test the query [here](https://ide.bitquery.io/Top-10-pump-fun-tokens-by-Price-change-in-last-5mins_2).
...
query MyQuery {
  Solana {
    DEXTradeByTokens(
      ...
      Price_Change_6h: calculate(expression: "(($Trade_CurrentPrice - $Trade_Price_6h_ago) / $Trade_Price_24h_ago) * 100")
    }
  }
}
```
