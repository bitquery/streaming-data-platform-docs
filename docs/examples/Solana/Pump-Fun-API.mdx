...
Here is a [query](https://ide.bitquery.io/pumpfun-tokens-which-crossed-10k-marketcap) that gives all tokens that crossed the 10k marketcap.
...
```
{
  Solana {
    DEXTrades(
      ...
      where: {Trade: {Buy: {PriceInUSD: {gt: 0.00001},
        Currency: {MintAddress: {notIn: ["11111111111111111111111111111111"]}}},
        Sell: {AmountInUSD: {gt: "10"}}, Dex: {ProtocolName: {is: "pump"}}}, Transaction: {Result: {Success: true}}}
    ) {
