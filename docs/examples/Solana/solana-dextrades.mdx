...
The GraphQL APIs and subscriptions below are examples of the real-time and historical trade data you can access across [Raydium](Solana-Raydium-DEX-API.mdx), [Orca](solana-orca-dex-api.mdx), [Phoenix](Solana-Phoenix-api.md), [Meteora](Meteora-DAMM-v2-API.md), [Jupiter](solana-jupiter-api.mdx), and other Solana-based DEXs.
...
- **[Solana Instructions API](solana-instructions.mdx)** - Track real-time instructions and token creation events
- **[Solana Balance Updates API](solana-balance-updates.mdx)** - Monitor wallet balance changes from trades
- **[Solana Transfers API](solana-transfers.mdx)** - Track token transfers and movements
- **[Solana Fees API](solana_fees_api.md)** - Analyze trading fees and transaction costs
- **[Solana Token Supply API](token-supply-cube.mdx)** - Monitor token supply changes from trading activities
...
For analyzing the transaction fees associated with these trades, see our **[Solana Fees API](solana_fees_api.md)**.
