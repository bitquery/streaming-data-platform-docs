...
Use below query to calculate the price change percentage for a specific token in last 5 min, 1 hr, 6hr. Test the query [here](https://ide.bitquery.io/Price-change-5min-1hr-6hr-precentage-of-a-specific-token).
...
query MyQuery {
  Solana {
    DEXTradeByTokens(
      where: {Transaction: {Result: {Success: true}}, Trade: {Currency: {MintAddress: {is: "vLieQF5eDqUuuk8RzRaqgAHkXr7bSEBZaWk9Zfibonk"}}, Market: {MarketAddress: {is: "9qppy1KXRTFEeWkFaysYHD7eu9GLg5pGXdLkdL51p7EX"}}}, Block: {Time: {since_relative: {hours_ago: 6}}}}
    ) {
      Trade {
      ...
      Price_Change_5min: calculate(expression: "(($Trade_CurrentPrice - $Trade_Price_5min_ago) / $Trade_Price_6h_ago) * 100")
      Price_Change_1h: calculate(expression: "(($Trade_CurrentPrice - $Trade_Price_1h_ago) / $Trade_Price_6h_ago) * 100")
...
Use the below query to get top 10 solana tokens by price change in last 1 hr. Test the query [here](https://ide.bitquery.io/Top-10-solana-tokens-by-price-change-in-last-1-hr#)
