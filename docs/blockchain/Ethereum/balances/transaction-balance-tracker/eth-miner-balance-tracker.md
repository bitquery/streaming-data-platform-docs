...
<FAQ
  items={[
    ...
    { q: "Where are the proposer's earnings?", a: "Builder payments to the proposer are regular transfers at the end of a block, covered by the MEV balance tracker. Consensus-layer rewards arrive as consensus-layer withdrawals, which are not transactions, so this cube does not record them and reason code 3 returns no rows." },
  ]}
/>
