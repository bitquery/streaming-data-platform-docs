...
description: "Polygon (MATIC) API Documentation: query and stream Polygon on-chain data with Bitquery GraphQL examples for developers."
...
:::tip Building a trading app or DEX UI on Polygon (MATIC)?
For **real-time trades and prices on Polygon (MATIC)** (and the last ~30 days), use the curated [**Trading cube**](/docs/trading/trading-data-overview) — [`Trading.Trades`](/docs/trading/crypto-trades-api/trades-api) gives you clean, MEV-filtered swaps with **USD price, market cap, and supply on every row** across **10 chains in one API**. For **historical Polygon (MATIC) data older than ~30 days**, use the chain-level `DEXTrades` / `DEXTradeByTokens` APIs documented below.
:::
...
- [Polygon (MATIC) Slippage API](./matic-slippage-api)
...
- [Polygon (MATIC) Liquidity API](./matic-liquidity-api)
...
- [Polygon (MATIC) Address Balance API](./matic-balance-api) — token and [native MATIC](./matic-balance-api#native-matic-balance) balances (`EVM.Balances`)
