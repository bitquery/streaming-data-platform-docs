...
Need low-latency BSC data? See [Streams](/docs/streams/) and contact us for a trial. For **mempool (pre-confirmation)** Four Meme data, see the dedicated [Four Meme Mempool API](/docs/blockchain/BSC/four-meme-mempool-API/) page.
...
query MyQuery($token: String) {
  EVM(network: bsc) {
    Balances(
      where: {
        Balance: { Address: { is: "0x5c952063c7fc8610FFDB798152D69F0B9550762b" } }
...
query MyQuery {
  EVM(network: bsc) {
    Balances(
      ...
      Balance {
        Amount(selectWhere: { ge: "200000000", le: "240000000" })
      }
    }
  }
}
...
You can refer to this [example](/docs/blockchain/BSC/four-meme-api/#subscribe-the-latest-trades-on-four-meme) to track latest trades of a token on other particular DEX's such as Pancake Swap.
...
query MyQuery {
  EVM(network: bsc) {
    Balances(
      ...
      Balance {
        Amount(selectWhere: { gt: "800000000", le: "900000000" })
      }
    }
  }
}
...
query MyQuery {
  EVM(network: bsc) {
    Balances(
      where: {
        Balance: { Address: { is: "0x5c952063c7fc8610FFDB798152D69F0B9550762b" } }
        Currency: { SmartContract: { is: "0x87c5b3da05b062480b55c2dbf374ccd084f74444" } }
      }
    ) {
      Currency {
        Name
...
| Can I monitor Four Meme transactions in the BSC mempool before confirmation? | **Separate guide:** [Four Meme Mempool API](/docs/blockchain/BSC/four-meme-mempool-API/) (not duplicated on this page)                                     |
...
- [Tutorial](/docs/streams/sniper-trade-using-bitquery-kafka-stream/)
