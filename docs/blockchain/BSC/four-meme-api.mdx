...
Below query gives you all time high price of a four meme token.
Try the query [here](https://ide.bitquery.io/four-meme---token-ATH-price).
...
query MyQuery($token: String!) {
  ...
  marketCap: Trading {
    Pairs(
      where: {Interval: {Time: {Duration: {eq: 1}}}, Market: {Network: {is: "Binance Smart Chain"}}, Volume: {Usd: {gt: 5}}, Token: {Address: {is: $token}}}
