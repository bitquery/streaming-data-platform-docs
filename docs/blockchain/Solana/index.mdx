...
## Why is Bitquery Solana data delayed compared to chain tip (e.g. several minutes behind)? {#why-is-bitquery-solana-data-delayed-compared-to-chain-tip}

Solana data in Bitquery is **indexed and served through a pipeline**; during high load, **catch-up**, or **operational work**, the **latest indexed slot** can lag **real time** by **minutes**. That is usually **ingestion delay**, not your query syntax. Use **`dataset: realtime`** for the **freshest GraphQL window**, check [Bitquery system status](https://app-status.bitquery.io/) for Solana, and use **managed streams** ([WebSockets](/docs/subscriptions/websockets/), [Kafka](/docs/streams/kafka-streaming-concepts/), [gRPC CoreCast](/docs/grpc/solana/introduction/)) if you need **lower-latency** delivery. Persistent gaps warrant [support](https://support.bitquery.io/).
