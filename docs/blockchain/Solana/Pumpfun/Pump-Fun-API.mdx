...
Returns all-time-high price and marketcap in USD (marketcap = price \* 1B tokens)
[ATH market cap of a Pump.fun token — Query ➤](https://ide.bitquery.io/Get-ATH-Market-Cap-of-a-Token#)
  ...
  Solana(dataset: combined) {
    DEXTradeByTokens(
      where: {
        Trade: {
          Currency: {
            MintAddress: { is: "3mZWpGvudc3rgDem4gNAW7K14A9CubYhG5ugDZ5npump" }
          }
        }
      }
      limit: { count: 1 }
    ) {
      ATH_Price: quantile(of: Trade_PriceInUSD, level: 0.98)
      ATH_Marketcap: calculate(expression: "$ATH_Price * 1000000000")
    }
  }
}
```
...
Same as above but within a given window timeframe.
[ATH price and market cap in timeframe — Pump.fun Query ➤](https://ide.bitquery.io/ATH-Market-Cap-in-a-Specific-Timeframe_2#)
  ...
  Solana(dataset: combined) {
    DEXTradeByTokens(
      where: {
        ...
        Block: {
          Time: { since: "2025-10-22T10:25:00Z", till: "2025-10-22T11:25:00Z" }
        }
      }
      limit: { count: 1 }
    ) {
