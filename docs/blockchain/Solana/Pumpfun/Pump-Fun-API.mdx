...
[Pump.fun token latest price in USD — Query ➤](https://ide.bitquery.io/Pumpfun-token-latest-price-USD)
...
```graphql
{
  Trading {
    Pairs(
      where: {Market: {Network: {is: "Solana"}}, Token: {Address: {is: "Ce2gx9KGXJ6C9Mp5b5x1sn9Mg87JwEbrQby4Zqo3pump"}}, Interval: {Time: {Duration: {eq: 60}}}}
      limit: {count: 1}
...
Use the below query to get top 10 Pump.fun tokens by price change in the last 5 minutes. Test the query [here](https://ide.bitquery.io/Top-10-pump-fun-tokens-by-Price-change-in-last-5mins_3).
...
```graphql
{
  Solana(dataset: combined) {
    DEXTradeByTokens(
      ...
      where: {Trade: {Currency: {MintAddress: {is: "66VR6bjEV5DPSDhYSQyPAxNsY3dgmH6Lwgi5cyf2pump"}}, Dex: {ProgramAddress: {is: "6EF8rrecthR5Dkzon8Nwu78hRvfCKubJ14M5uBEwF6P"}}}}
    ) {
      Block {
...
[Trading volume of a Pump.fun token — Query ➤](https://ide.bitquery.io/trading-volume-of-a-token-pump-fun-lat-7-days)
  ...
  Trading {
    Tokens(
      where: {
        ...
        Token: { Address: { is: "cbbtcf3aa214zXHbiAZQwf4122FBYbraNdFqgw4iMij" } }
      }
    ) {
      sum(of: Volume_Usd)
    }
...
### How do I get Pump.fun tokens between 400K–450K market cap with dev and creation time?

Filters tokens in a specific cap range and adds metadata
[Pump.fun tokens between 400K–450K market cap — Query ➤](https://ide.bitquery.io/pumpfun-tokens-between-400K-and-450K-MC-and-their-creation-date-dev-address_3)
