...
We use the Solana v1 Transfers API for this. This query finds "Pump Fun" token launches in Mayhem mode by filtering the count of transfers of exactly 1,000,000,000,000,000 units (1 Billion if adjusted to 6 decimal places).
If count is 2 it means token is minted in mayhem mode and if count is 1 then it means token is not minted in mayhem mode.
