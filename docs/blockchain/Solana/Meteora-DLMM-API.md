...
| Rate limits | 30 requests a second | 30, 90 or 240 requests a minute by plan, custom on Enterprise |
...
## Real-time price feed of a token on Meteora DLMM
...
To stream liquidity changes across all DLMM pools, subscribe to the same cube with `Base: { ChangeAmount: { ne: "0" } }`. Each message is one reserve change with the pool, both currencies and the post-event balances.
...
## Top traders of a token on Meteora DLMM
...
## Trading volume, buy volume and sell volume of a token on Meteora DLMM
...
<FAQ
  items={[
    ...
    { q: "How do I get OHLC candles for a Meteora DLMM pair?", a: "Aggregate DEXTradeByTokens by a time interval as shown in the OHLC section, or read pre-built candles from Trading.Pairs filtered by Market.Protocol lb_clmm with Price.IsQuotedInUsd true. Run both as queries, not subscriptions." },
