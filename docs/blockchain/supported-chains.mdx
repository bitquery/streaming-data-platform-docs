...
**Arc mainnet** is live in V2 GraphQL, WebSocket subscriptions and the cross-chain `Trading` cubes. For trade streams and the last 30 days, use `Trading.Trades` with `Pair.Market.Network: "Arc"`; use `Trading.Tokens` or `Trading.Pairs` for prices and candles. Use EVM trade cubes only for trades older than 30 days, once Arc archive support is confirmed. Other chain data uses `EVM(network: arc)`; see the [Arc Mainnet API guides](/docs/blockchain/arc-mainnet/). The matrix does not list chain-level Kafka, cloud or ClickHouse coverage for Arc mainnet.
