...
description: "The Balances API returns current and historical token balances for an address on Base. To return only non-zero balances, add Amount(selectWhere: { gt: \"0\" }) on the Balance field (not in where). Use dataset: combined or dataset: archive as follows:."
---
