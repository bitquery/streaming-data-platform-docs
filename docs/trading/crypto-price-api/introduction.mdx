...
These tools let you stream and query aggregated price data in USD or other paired currencies based on time and volume for all tokens across EVM, Solana, Tron, and other supported chains. Our **crypto prices api** is the **main** source for **real-time** pricing and **OHLC**; for **historical** OHLC from DEX trades, use **`DEXTradeByTokens`** as described in the [OHLC documentation](https://docs.bitquery.io/docs/trading/crypto-price-api/crypto-ohlc-candle-k-line-api/).
...
## How do I get the current price of a token using Bitquery API? {#how-do-i-get-the-current-price-of-a-token-using-bitquery-api}

Subscribe or query the **`Trading`** API: use **`Tokens`**, **`Pairs`**, or **`Currencies`** with your token or pair identifiers and read **`Price`**, including **`Ohlc.Close`** for the latest bar, over your chosen **`Interval`**. Use the streaming endpoint and a WebSocket client as in [Quick start](#quick-start), or run the same shape as a **query** for a snapshot. See the [Tokens cube](https://docs.bitquery.io/docs/trading/crypto-price-api/tokens/) and [examples](https://docs.bitquery.io/docs/trading/crypto-price-api/examples/).

## Is there an API to get token price in USD on Solana? {#is-there-an-api-to-get-token-price-in-usd-on-solana}

**Yes.** Solana is a first-class network in the **Crypto Price API**: filter `Trading.Tokens` (or `Pairs`) by **`Token.Network: solana`** and use **`Price.IsQuotedInUsd`** / **`Price.Ohlc`** for aggregated USD. For **DEX-reported** USD from raw trades, use **`Solana { DEXTradeByTokens { ... Trade { ... PriceInUSD } } }`** on the [Solana DEX Trades API](https://docs.bitquery.io/docs/blockchain/Solana/solana-dextrades/#latest-usd-price-of-a-token).
## How do I get price change percentage for a token? {#how-do-i-get-price-change-percentage-for-a-token}

Compare **two prices** in one query—e.g. **`PriceInUSD`** at **`maximum: Block_Time`** vs at an earlier time using **`if: { Block: { Time: { till_relative: { hours_ago: 24 } } } }`**—then use a **`calculate`** expression for **(current − past) / past × 100**. See [GraphQL expressions](https://docs.bitquery.io/docs/graphql/capabilities/expression/) and the Solana walkthrough [Price change 5min, 1hr, 6hr](https://docs.bitquery.io/docs/blockchain/Solana/solana-dextrades/#price-change-5min-1hr-6hr-precentage-of-a-specific-token).
...
```graphql
{
  Trading {
    Currencies(
      ...
      where: {Currency: {Id: {is: "bid:bitcoin"}}, Volume: {Usd: {gt: 5}}}
    ) {
      Volume {
        Usd
        Quote
...
```graphql
{
  Trading {
    Tokens(
      where: {Token: {Address: {is: "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v"}, Network: {is: "Solana"}}, Interval: {Time: {Duration: {eq: 1}}}, Volume: {Usd: {gt: 5}}}
      limit: {count: 1}
      orderBy: {descending: Block_Time}
    ) {
      Token {
...
subscription {
  Trading {
    Pairs(
      where: {Market: {Network: {is: "Base"}}, QuoteToken: {Address: {is: "0x4200000000000000000000000000000000000006"}}, Token: {Address: {is: "0x940181a94a35a4569e4529a3cdfb74e38fd98631"}}, Volume: {Usd: {gt: 5}}}
    ) {
      Currency {
        Symbol
        Name
...
Get volume and price change data for multiple tokens trading on different chains (Solana, Ethereum, BSC, Tron) in a single query. Returns volume for 1h, 4h, and 24h periods, plus price change percentages for the same intervals.
