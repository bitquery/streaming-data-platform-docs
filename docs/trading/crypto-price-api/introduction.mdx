...
- **Granularity**:
  ...
  - The **Crypto Price API (Price Index)** provides **pre-aggregated price metrics** such as [OHLC (Open, High, Low, Close)](../crypto-ohlc-candle-k-line-api/) and average prices, calculated over volume- or time-based intervals—as granular as 1 second.

- **Aggregation**:

  - In DEX APIs, you had to **aggregate trades** to calculate OHLC or moving averages.
  - The Crypto Price API (Price Index) provides **ready-to-use [OHLC and price statistics](../crypto-ohlc-candle-k-line-api/)** out of the box , no need to calculate anything.
