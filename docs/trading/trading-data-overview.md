...
| **Chains** | Each chain has its own root (EVM, Solana, Tron, etc.) | **10 chains under one API**: Ethereum, BSC, Solana, Base, Arbitrum, Tron, Optimism, Polygon (`Matic`), Robinhood, Arc |
...
It is exposed under a single `Trading` root and covers **10 chains in one API**: Ethereum, BSC, Solana, Base, Arbitrum, Tron, Optimism, Polygon, Robinhood, and Arc.
...
```
   ...
   └────────────────────┬─────────────────────────────────────────┘
                        ...
                        ▼
   ┌──────────────────────────────────────────────────────────────┐
   │  Trading Cube  (10 chains under one API)                      │
...
| Stream all swaps on 10 chains in **one** subscription | `Trading.Trades` |
...
<FAQ
  items={[
    { q: "When should I use Trading.Trades instead of DEXTrades?", a: "Use Trading.Trades for real-time or last ~30 days when you want clean USD prices, market cap, and MEV-filtered swaps across 10 chains in one query. Use DEXTrades when you need call/event context or raw per-chain detail." },
    ...
    { q: "Which chains does Trading.Trades cover?", a: "Solana, Ethereum, BSC, Base, Arbitrum, Optimism, Polygon, Tron, Robinhood and Arc in one schema. API filter values are case-sensitive display names: Polygon is 'Matic', BSC is 'Binance Smart Chain', and Arc is 'Arc'." },
  ]}
/>
