...
keywords:
  ...
  - Hyperliquid tax data
  - Hyperliquid tax report
...
Queue position, order lifetime, maker behaviour, spoofing patterns, and _why_ a liquidation fired are only recoverable from this shape of data.

## Hyperliquid Data for Tax Reporting

Tax reports need a wallet's **complete** trading history for the year — not the recent window most exports and APIs return. Bitquery's Hyperliquid dataset has every event since launch, so you, your accountant or your tax software can compute gains, losses and income for any wallet and any date range.

Everything a Hyperliquid tax computation needs is in the dataset:

| Tax input                                               | Bitquery dataset                                             |
| ------------------------------------------------------- | ------------------------------------------------------------ |
| Realized PnL on every closed position                   | `fills` — per-fill closed PnL                                |
| Trading fees, including builder and HIP-3 deployer fees | `fills` — fee, fee token, builder fee, deployer fee          |
| Funding paid and received                               | `misc_events` — funding                                      |
| Liquidations                                            | `fills` — liquidation detail                                 |
| TWAP executions, fill by fill                           | `fills` — TWAP id, with `twap_statuses` for the parent order |
| Spot buys and sells for cost basis                      | `fills` on spot markets                                      |
| Deposits, withdrawals and transfers                     | `misc_events` — ledger updates and withdrawals               |

Why teams use it for tax work:

- **Full history, not a rolling window.** Every wallet's fills and ledger events back to Hyperliquid's launch, so a full tax year — or several — is always available.
- **Nothing dropped.** Funding, TWAP child fills and liquidations are included alongside regular trades, so balances and PnL reconcile.
- **Exact amounts.** Prices, sizes, PnL and fees are stored as strings, so no precision is lost to floating point.
- **Works with your stack.** Delivered to S3, GCS, Snowflake or BigQuery — query one wallet or thousands with SQL, then export to CSV for tax software.

Built for tax platforms, accountants and funds reconciling Hyperliquid activity, as well as individual traders with high trade counts. To get the full dataset, [fill the API form](https://bitquery.io/forms/api) or email **sales@bitquery.io**.
...
An L2 snapshot tells you the book was 12 deep at a price; an L4 diff stream tells you _which_ order moved and _whose_ it was. Likewise, `order_statuses` includes orders that **never traded** — rejects and cancels are invisible in any trades-only dataset, and in the sample below they are 96% of all order events.
...
`order_statuses` is a record of _intent_, and most intent never trades. The sample's 142,955 order events break down as:
